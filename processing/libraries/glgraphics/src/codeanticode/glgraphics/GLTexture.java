...
public class GLTexture extends PImage implements PConstants, GLConstants 
{
	...
	public void loadTextureFast()
	{
		...
		IntBuffer texels = gl.glMapBufferARB(pboTarget, GL.GL_WRITE_ONLY).asIntBuffer();
		texels.put(pixels);
    ...
    */    
    ...
    public void putBuffer(int[] intArray, int format, int type)
    {
        ...
        if (format == TEX1)
        {
        	...
        	if (type == TEX_UBYTE) convArray = convertToRGBA(intArray, ALPHA);
        }
        else if (format == TEX3)
        {
        	glFormat = GL.GL_RGB;
        	if (type == TEX_UBYTE) convArray = convertToRGBA(intArray, RGB);
        }
        else
        {
        	glFormat = GL.GL_RGBA;
        	if (type == TEX_UBYTE) convArray = convertToRGBA(intArray, ARGB);
        }
    ...
    public void putBuffer(float[] floatArray, int format)
    {
        ...
        int glFormat;
        if (format == TEX1) glFormat = GL.GL_LUMINANCE;
        else if (format == TEX3) glFormat = GL.GL_RGB;
    ...
    protected void setTextureParams(GLTextureParameters params)
    {
	    ...
	    else if (params.target == TEXRECT)
        {
            texTarget = GL.GL_TEXTURE_RECTANGLE_ARB;		
        }
	    else if (params.target == TEX1D)
        {
