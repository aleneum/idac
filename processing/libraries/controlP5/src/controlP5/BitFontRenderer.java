...
//	protected int _mySpacing = 0;
		...
		for (int f = theStart; f < theEnd; f++) {
			...
			for (int i = 0; i < characters[f]; i++) {
				for (int u = 0; u < charWidth[f][i] * charHeight[f]; u++) {
					chars[f][i][u] = font[f].pixels[indent + (u / charWidth[f][i]) * font[f].width
					        + (u % charWidth[f][i])];
				}
	...
	protected int getWidth(
		...
		for (int i = 0; i < theLength; i++) {
			...
			if (myIndex >= 0 && myIndex<=95) {
				...
				System.out.println("WARNING, you are using a character that is not supported by controlP5's BitFont-Renderer, you could use ControlFont instead (see the ControlP5controlFont example).");
			}
		}
		return myWidth;
	}
	...
	private void putchar(
		...
		for (int i = 0; i < len; i++) {
			...
			if (chars[theFontIndex][theC][i] == 0xff000000 && xpos < theImage.width && xpos >= 0 && pos >= 0
			        && pos < myWH) {
		...
		for (int i = 0; i < letters.length; i++) {
			if (letters[i].letter != -1) {
				putchar(letters[i].letter, theLabel.getOffsetX() + letters[i].indent, letters[i].lineheight, theLabel
				        .getColor(), letters[i].isHighlight, theLabel.getImage(), theLabel.getImageMask(), theLabel
				        .getFontIndex());
			}
		}
